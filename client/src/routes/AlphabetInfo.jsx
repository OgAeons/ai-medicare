...
function AlphabetInfo() {
    ...
    function handleLetterClick(letter) {
        if (letter !== selectedLetter) {
            ...
            setSearchQuery('');
            axios.get(`/api/diseases?letter=${letter}`)
                .then(response => {
                    console.log('API Response:', response.data); // Log the response data
                    setFilteredDiseases(response.data); // Ensure this is the correct state variable
                })
                .catch(error => {
    ...
    const filteredDiseasesList = searchQuery
        ? filteredDiseases.filter(disease =>
