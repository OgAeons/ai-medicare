...
function MapComponent({ doctors }) {
    ...
    function handleMarkerClick(doctor) {
  ...
  return (
    <div className='flex h-[50vh] w-full'>
        <div className={`transition-all duration-300 ${selectedDoctor ? 'w-[70%]' : 'w-full'}`}>
            <MapContainer
        ...
        {selectedDoctor && (
            <div className="w-[30%] bg-white p-4 shadow-md">
                ...
                <p><strong>Fees:</strong> ₹{selectedDoctor.consultationFees}</p>
            </div>
        )}
    </div>
  )
